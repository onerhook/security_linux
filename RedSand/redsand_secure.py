...
class RedSandSecure:
    ...
    def run_dynamic_analysis(self, file_path, timeout=60):
        ...
        # Проверка: является ли файл исполняемым скриптом
        if not os.path.isfile(file_path):
            print(f"[-] Файл не найден: {file_path}")
            return [{'error': 'File not found'}]

        # Определяем интерпретатор для запуска
        interpreter = None
        ...
        if file_ext == '.py':
            ...
            interpreter = []  # Запускаем напрямую
        else:
            # Для остальных типов файлов пытаемся запустить напрямую или через интерпретатор по умолчанию
            print(f"[*] Неизвестный тип файла {file_ext}, попытка прямого запуска")
            interpreter = []
    ...
    def analyze(self, file_path: str, use_poly: bool = False, timeout: int = 60) -> Optional[Dict[str, Any]]:
        """
        Публичный метод для анализа одного файла.
        Используется GUI и другими внешними модулями.

        Args:
            file_path: Путь к файлу для анализа
            use_poly: Использовать ли полиморфный анализ
            timeout: Таймаут динамического анализа (сек)

        Returns:
            Словарь с результатами анализа или None при ошибке
        """
        self.logger.info(f"Запуск анализа файла: {file_path}")
        result = self.analyze_single_file_internal(file_path, use_poly, timeout)

        if result.get('success'):
            self.logger.info("Анализ завершен успешно")
        else:
            error_msg = result.get('error', 'Неизвестная ошибка')
            self.logger.error(f"Ошибка анализа: {error_msg}")
    ...
    def analyze_single_file_internal(self, file_path: str, use_poly: bool = False, timeout: int = 60) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Ошибка в analyze_single_file_internal: {error_trace}")
            return {
                'file_path': file_path,
                'file_hash': file_hash,
                'file_size': file_size,
